...
int Simulation::run(int nsteps) {  // perform "nsteps" steps of MD
  if ((n_search == nullptr) || (integrator == nullptr))
    return EXIT_FAILURE;                      // check if the simulation is set up correctly
  ...
  for (int i = 0; i < nsteps; i++) {
    ...
    for (auto& particle : system.particles)
      particle.reset();  // set force, stress, Ep, Ek, ro equal to zero before the next force calculation
    ...
    Force::force_3b(system, n_search->get_neighbours());     // calculate forces and potential energies for 3 body
...
int Simulation::add_handler(const Simulation::Handler_ptr& handler) {  // attach a handler to the simulation, which will
                                                                       // be called after each MD step
...
int Simulation::set_integrator(
    const Simulation::Integrator_ptr& integrator_) {  // set an integrator used in the simulation
...
System& Simulation::get_system() { return system; }
...
void Simulation::print(std::ostream& os) const {  // print information about the simulation setup into os stream
  ...
  if (system.get_ntypes() > 0) {
    ...
    for (int i = 1; i < system.get_ntypes(); i++) os << ", m[type" << i + 1 << "] = " << system.masses[i] << " Da";
    os << std::endl;
  }
  if (n_search != nullptr)
    os << n_search->info() << std::endl;  // print information about the neighbour search algorithm
  else
    os << "neighbour search algorithm is not set" << std::endl;
  if (integrator != nullptr)
    os << integrator->info() << std::endl;  // print information about the integration algorithm
  else
    os << "Integration algorithm is not set" << std::endl;
  if (!handlers.empty()) {
  ...
  for (int i = 1; i <= ntypes; i++) {
    for (int j = i; j <= ntypes; j++) {
      ...
      os << "type" << i << "-type" << j << " : ";
      if (system.potentials[id] != nullptr)
        os << system.potentials[id]->info();
      else
        os << "null";
      os << std::endl;
    }
...
void Simulation::gather() {  // put particles back into the computational cell when periodic boundary conditions are
                             // used
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < DPVector3::dim; j++) {  // dim = 3
      ...
      if (pos.data[j] < 0.0)
        pos.data[j] += system.size[j];  // put particles back into the computational cell
      else if (pos.data[j] > system.size[j])
        pos.data[j] -= system.size[j];
    }
  }
}
...
void Simulation::stress() {  // calculate Sxx, Syy, and Szz components of stress tensor for each atom
  double volume;
  if (system.dim == 2)
    volume = system.XL * system.YL * 1e-20;
  else
    volume = system.XL * system.YL * system.ZL * 1e-30;  // volume of the computational domain
  for (auto& particle : system.particles) {              // loop over all particles
    double m =
        system.masses[particle.type -
