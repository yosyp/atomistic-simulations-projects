...
class Simulation {
 public:
  ...
  using Integrator_ptr = std::shared_ptr<Integrator_Base>;  // An abbreviated name for a smart pointer to
                                                            // Integrator_Base
  ...
  int add_handler(const Handler_ptr &handler);  //!< Attach a handler to the simulation, which will be called after each
                                                //!< MD step. Check Handlers.h for more details.
 ...
 private:
  ...
  std::vector<Handler_ptr> handlers; /*!< An array of smart pointers to handlers called after each MD step. Check
                                        Handlers.h for more details.*/
