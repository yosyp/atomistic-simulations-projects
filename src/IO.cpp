...
int Reader_Data::load(System & system) {										//load system form a restarting (.data) file
	...
	for (int i = 0; i < n; i++) file >> particles[i].khist >> particles[i].pos.x >> particles[i].pos.y >> particles[i].pos.z;
...
//Reader_CG==================================================================================================
//initialize Reader_CG, *"ncells" is the number of unit cells{ nx, ny, nz }, "a_lat" is the lattice constant, "lattice" is the crystal structure of the generated system
Reader_CG::Reader_CG(const std::array<int, 3>& ncells_, double a, Lattice lattice) {
	...
	case Lattice::SC:
		...
		types = { 1 };
		basis = { { 0.0, 0.0, 0.0 } };
		shift.x = 0.5; shift.y = 0.5; shift.z = 0.5;
	...
	case Lattice::BCC:
		...
		shift.x = 0.25; shift.y = 0.25; shift.z = 0.25;
	...
	case Lattice::Diamond:
		...
		shift.x = 0.125; shift.y = 0.125; shift.z = 0.125;
...
int Reader_CG::load(System & system) {											//load system form a Crystal Generator pseudo file
	...
	std::cout << "Building " << lattice_type << " crystal with nx = " << ncells[0] << ", ny = " << ncells[1] << ", nz = " << ncells[2] << std::endl;
		...
		for (int iy = 0; iy < ncells[1]; iy++) {
			for (int ix = 0; ix < ncells[0]; ix++) {
				for (int ic = 0; ic < cell_n; ic++) {
					...
					particles[id].pos = (basis[ic] + shift + DPVector3(static_cast<double>(ix), static_cast<double>(iy), static_cast<double>(iz)))*a_lat;
...
int Writer_Snapshot::save(const System& system) {								//Save atomic snapshot (.d) file
	...
	auto& particles = system.particles;
	int n = particles.size();													//number of particles
	for (int i = 0; i < n; i++) {												//write: step, khist, pos[0-2] [A], vel[0-2] [A/ps], Ep [eV], Ek [eV], T [K], Etot [eV], type
		double Etot = particles[i].Ep + particles[i].Ek;
		double T = 2.0*particles[i].Ek*kb / 3.0;
		file << i << " " << particles[i].khist << " " << particles[i].pos.x << " " << particles[i].pos.y << " " << particles[i].pos.z << " "
			<< particles[i].vel.x << " " << particles[i].vel.y << " " << particles[i].vel.z << " "
			<< particles[i].Ep << " " << particles[i].Ek << " " << T << " " << Etot << " " << particles[i].type << std::endl;
	}
	return EXIT_SUCCESS;
}