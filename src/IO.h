 ...
 *  It includes reading/writing restarting files, writing atomic snapshots, and reading from crystal generator pseudo
 * file. */
...
class Reader_CG {
 public:
  ...
  Reader_CG(const std::array<int, 3>& ncells, double a_lat,
            Lattice lattice); /**< Initialize Reader_CG,
...
/*! A writer capable to save atomic snapshots, (.d) files: step, khist, pos[0-2] [A], vel[0-2] [A/ps], Ep [eV], Ek [eV],
 * T [K], Etot [eV], type */
class Writer_Snapshot {
 public:
  Writer_Snapshot(
      const std::string& filename);  //!< Initialize Writer_Snapshot, "filename" is the name of the saved file
