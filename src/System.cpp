...
System::System()
    : XL(0.0),
      ...
      Xcenter(0.0),
      Ycenter(0.0),
      Zcenter(0.0),  // set default values
      boundaries{Boundary::periodic, Boundary::periodic, Boundary::periodic},
      particles(),
      masses(),
      potentials(),
      nptypes(0),
...
void System::set_ntypes(int n) {  // set the number of particles types
  ...
  std::fill(potentials.begin(), potentials.end(), Potential_ptr());  // fill array of potentials with null pointer;
                                                                     // after "set_ntypes(n)" call, potentials must be
                                                                     // reassigned
}

int System::get_ntypes() const { return nptypes; }

int System::get_n_interactions() const {  // return the number of interaction types for the given number of particle
                                          // types: (type1, type1), (type2, type2), (type1, type2), etc.
...
bool System::verify() {  // if system is set up correctly, returns true
  ...
  for (int i = 0; i < nptypes; i++) {
    for (int j = i + 1; j < nptypes; j++) {
      ...
      if ((potentials[id] != nullptr) &&
          potentials[id]->eam() &&  // if cross interaction ij is eam, interactions ii and jj must be also eam
          ((potentials[i] == nullptr) || !potentials[i]->eam() || (potentials[j] == nullptr) ||
           !potentials[j]->eam())) {
      ...
      if ((potentials[id] != nullptr) &&
          potentials[id]
              ->threebody() &&  // if cross interaction ij is 3 body, interactions ii and jj must be also 3 body
          ((potentials[i] == nullptr) || !potentials[i]->threebody() || (potentials[j] == nullptr) ||
           !potentials[j]->threebody())) {
