...
* @date 24 Jan 2018
* @brief The file contains implementation of algorithms for calculation of interatomic interaction energies and forces for pair, EAM, and 3 body potentials, declared in Force.h */
...
int Force::eam_density(System& system, const Narray& neighbours) {				//update EAM density
	int n_interactions = system.get_n_interactions();							//number of different interatomic interactions: (type1, type1), (type2, type2), (type1, type2), etc.
...
//	Calculation of electron density only, applied only for EAM potentials
	for (int interaction = 0; interaction < n_interactions; interaction++) {	//loop over all interactomic interactions present in the system, like (type1, type1), (type2, type2), (type1, type2), etc.
		auto& potential = potentials[interaction];								//an abbreviated name
		if ((potential == nullptr) || (!potential->eam())) continue;			//check if a particular interaction involves calculation of electron density; if potential is not set, particles do not interact
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j_id = 0; j_id < n_neighbours; j_id++) {
				...
				p1.ro += potentials[p2.type - 1]->f().get(r);					//add electron density to particle1; particle type ids start from 1 wile indexes in array start form 0, therefore "-1"
...
//	Electron contribution to the total energy, applied only for EAM potentials
	for (int i = 0; i < n; i++) {
		auto& potential = system.potentials[particles[i].type - 1];				//interaction potential; particle type ids start from 1 wile indexes in array start form 0, therefore "-1"
		if ((potential != nullptr) && (potential->eam())) {						//skip if interaction of atoms i is not described by EAM potential
			particles[i].Ep += potential->F().get(particles[i].ro);				//add F(ro) contribution to the potential energy
			particles[i].ro = potential->F().get_d(particles[i].ro);			//replace variable ro by dF(ro), to avoid calculation of dF/dro for each atomic pair during force calculation
		}
	}
	return EXIT_SUCCESS;
}
	...
	for (int interaction = 0; interaction < n_interactions; interaction++) {	//loop over all interactomic interactions present in the system, like (type1, type1), (type2, type2), (type1, type2), etc.
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j_id = 0; j_id < n_neighbours; j_id++) {
				...
				double force, U;
				potential->U().get_tot(r, U, force);							//get values of U and dU/dr from the table
																				//if the considered interaction is EAM, add an electron density contribution to the force
				if (potential->eam()) force += p1.ro*potentials[p2.type - 1]->f().get_d(r)
...
int Force::force_3b(System& system, const Narray& neighbours) {					//calculate forces and potential energies for 3 body interaction part, if one is present
	int n_interactions = system.get_n_interactions();							//number of different interatomic interactions: (type1, type1), (type2, type2), (type1, type2), etc.
	...
	for (int interaction_ij = 0; interaction_ij < n_interactions; interaction_ij++) {	//loop over all interactomic interactions present in the system, like (type1, type1), (type2, type2), (type1, type2), etc.
		auto& potential_ij = potentials[interaction_ij];
		if ((potential_ij == nullptr) || (!potential_ij->threebody())) continue;	//check if a particular interaction is defined and involves calculation of 3 body term 
		double Rcut2_ij = pow(potential_ij->r_cut(), 2);						//square cutoff distance
		for (int interaction_ik = interaction_ij; interaction_ik < n_interactions; interaction_ik++) {	//loop over interactomic interactions present in the system
			auto& potential_ik = potentials[interaction_ik];
			if ((potential_ik == nullptr) || (!potential_ik->threebody())) continue;	//check if a particular interaction is defined and involves calculation of 3 body term 
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j_id = 0; j_id < n_neighbors_ij; j_id++) {
					...
					int k_id_start = (interaction_ij == interaction_ik) ? j_id + 1 : 0;  //start "k" loop from "j_id + 1" for interaction between atoms of the same kind
					for (int k_id = k_id_start; k_id < n_neighbors_ik; k_id++) {		//otherwise start from 0
						...
						apply_bounday(dr_ik, system);							//correct the interatomic distance if periodic boundary conditions are applied
						...
						potentials[type_i - 1]->tb_g().get_tot(cos_jik, g_cos_jik, dg_cos_jik); //angle dependent part of the interaction
						potential_ik->tb_f().get_tot(r_ik, f_ik, df_ik);		//radial dependent parts of the interaction, types start from 1, therefore "-1"
