 ...
 *  It includes calculation of Energy, Temperature, and Pressure as well as assignment of initial velocity and using
...
class Handler_Base {
 ...
 private:
  virtual void update_impl(System &system) = 0;  //!< This function implements a method updating the system. Override
                                                 //!< this function to implement a particular algorithm

  bool active;  /*!< A flag that determines if the handler is active */
  int update_n; /*!< The frequency of system update */
};

/*! This class is responsible for assignment of the velocity to particles according to the Maxwell distribution. 3KT
   energy is assigned per particle, and half of it will be transferred to the potential energy (I assume that the
   initial configuration is quenched). More about initialization of velocities in Frenkel & Smit pp 56-57 */
class Handler_Velocity : public Handler_Base {  // this class implements Handler_Base interface
 public:
  Handler_Velocity(
      double T0 = 300.0);  // Create a handler assigning T0 by setting particle velocities at the 0-th time step
...
class Handler_Temperature : public Handler_Base {  // this class implements Handler_Base interface
 public:
  ...
  void exclude_groups(
      const std::vector<int> &list);  //!< Set a list of groups (khist) that are excluded during temperature calculation
 protected:
  virtual void update_impl(System &system) override;  //!< Calculate the Temperature [K], estimated based on the average
                                                      //!< atomic kinetic energy: <Ek> = 1.5*kb*T
  bool exclude(int group) const;                      //!< Check if a particular group in the exclusion list

  double T;                      /*!< The Temperature of the system */
  std::vector<int> exclude_list; /*!< The exclusion list. Atoms with the group (khist) within the list are skipped
                                    during calculations */
};
...
class Handler_BerendsenT : public Handler_Temperature {  // this class expands Handler_Temperature
 public:
  Handler_BerendsenT(double T, double tau = 2.0);  //!< Initialize Handler_BerendsenT object. T is the target
                                                   //!< temperature, and tau is the time constant that defines the
                                                   //!< strength of coupling to the thermal bath
 ...
 protected:
  ...
  double tau; /*!< The time constant [ps] that defines the strength of coupling to the thermal bath. It is usually
                 chosen from the range of 0.1 - 2 ps */
};

/*! This class implements Berendsen barostat algoritm. At each time step the system size is scaled to maintain the
 * target temperature. Check "Berendsen, J.Chem.Phys., 81 (1984), p. 3684" for more details. */
class Handler_BerendsenP : public Handler_Pressure {  // this class expands Handler_Pressure
 public:
  ...
  Handler_BerendsenP(double P, Mode mode = Mode::XYZ, double beta = 1e-10); /**<  Initialize Handler_BerendsenP object.
                                                                             ...
                                                                             * compressibility, dt - time step, tp -
                                                                             * characteristic time of pressure
                                                                             * relaxation)*/
 ...
 protected:
  ...
  double beta; /*!< beta = Kt*dt/(3*tp) [Pa^-1] (Kt - isothermal compressibility, dt - time step, tp - characteristic
                  time of pressure relaxation */
  Mode mode;   /*!< The method of pressure control, check enum class Mode for more details */
};

/*! This class is responsible for heating of the material in a less violent manner then Handler_Velocity. However, a
 * small value of initial velocity must still be assigned */
class Handler_Heating : public Handler_Temperature {  // this class expands Handler_Temperature
 public:
  Handler_Heating(double T, double tau = 1.0);  //!< Initialize Handler_Heating object. T is the target temperature, and
                                                //!< tau is a characteristic time [ps] of heating
...
/*! This class implements a Quench algorithm that allows to stop motion of atoms and bring system to the minimum (local
 * or global) of the potential surface */
