...
Potential_Base::Potential_Base()
    : full_list_flag(false), eam_flag(false), tb_flag(false), cut_off(0.0), name(), build_flag(false), mass(1.0) {}

bool Potential_Base::is_build() const { return build_flag; }

double Potential_Base::get_mass() const { return mass; }
...
void Potential_Base::print() const {  // print tables into a file: r[A], U[eV], dU/dr[eV/A]...
  ...
  if (eam_flag)
    ro1 = F_table.min(), ro2 = F_table.max();  // minimum and maximum values of electron density for EAM potentials
  if (tb_flag)
    costh1 = tb_g_table.min(),
    costh2 = tb_g_table.max();  // minimum and maximum values of electron density for EAM potentials
  ...
  for (int i = 0; i < n_points; i++) {
    ...
    if (eam_flag) {
      ...
      out << " " << f_table.get(r) << " " << f_table.get_d(r) << " " << ro << " " << F_table.get(ro) << " "
          << F_table.get_d(ro);
    }
    if (tb_flag) {
      double costh =
          costh1 + (costh2 - costh1) * i /
                       (n_points - 1);  // if 3 body potential, print f (radial dependent part of 3 body term), df/dr,
                                        // cos[theta], g(angular dependent part of the interaction), dg/d(cos[theta])
      out << " " << tb_f_table.get(r) << " " << tb_f_table.get_d(r) << " " << costh << " " << tb_g_table.get(costh)
          << " " << tb_g_table.get_d(costh);
    }
    out << std::endl;
  }
}
...
Potential_LJ::Potential_LJ(Material material, double Rcut)
    : Rcut0(Rcut) {  // initialize Potential_LJ for a given "material", Rcut is the cutoff distance in units of sigma
...
double Potential_LJ::get_epsilon() const { return epsilon; }

double Potential_LJ::get_sigma() const { return sigma; }
...
Potential_LJ_cut::Potential_LJ_cut(
    Material material,
    double Rcut) {  // initialize Potential_LJ for a given "material", Rcut is the cutoff distance in units of sigma
  Rcut0 = Rcut;
  set_parameters(material);  // assign "epsilon", "sigma", "name", and "mass" for a given material
...
std::string Potential_LJ_cut::info() const {  // return a string with information about the potential
  return "Lennard-Jones potential with cutoff function for " + name +
         " interaction, Rcut = " + std::to_string(cut_off) + " A";
}

void Potential_LJ_cut::build() {  // assign values of U and dU tables for given LJ potential parameters with using
                                  // cutoff function
  ...
  for (int i = 0; i < n_points; i++) {
    ...
    dU[i] = -(epsilon * (48.0 / r) * sr6 * (sr6 - 0.5) -
              epsilon * 24.0 * src6 * (2.0 * src6 - 1.0) * r / (cut_off * cut_off));
  }
...
// Potential_EAM_alloy==================================================================================================
// genrate EAM potential based on an *.alloy file with a name "filename", "El1" and "El2" are names of elements that
// should be loaded form the file
Potential_EAM_alloy::Potential_EAM_alloy(const std::string &filename, const std::string &El1, const std::string &El2) {
  ...
  double dro, dr, r_max;
  in >> n_points_ro >> dro >> n_points_r >> dr >>
      r_max;  // the number of points in tables tabulated based on ro, dro,
  ...
  for (int i = 0; i < ntypes; i++) {  // calculate derivatives for F and f
    for (int j = 1; j < n_points_ro - 1; j++)
      dF[i][j] = (F[i][j + 1] - F[i][j - 1]) / (2.0 * dro);  // dF(i)/dr = (F(i+1) - F(i-1))/2dr
    dF[i][0] = dF[i][1];
    dF[i][n_points_ro - 1] = dF[i][n_points_ro - 2];  // set derivatives at boundaries
    for (int j = 1; j < n_points_r - 1; j++) df[i][j] = (f[i][j + 1] - f[i][j - 1]) / (2.0 * dr);
    df[i][0] = df[i][1];
    df[i][n_points_r - 1] = 0.0;  // set derivatives at boundaries
  }
  for (int i = 0; i < n_interactions; i++) {  // calculate derivatives for U
    for (int j = 1; j < n_points_r - 1; j++) dU[i][j] = (U[i][j + 1] - U[i][j - 1]) / (2.0 * dr);
    dU[i][0] = dU[i][1];
    dU[i][n_points_r - 1] = 0.0;  // set derivatives at boundaries
  }
  ...
  for (int i = 0; i <= std::min(material1, material2);
       i++) {                           // for 2 material types: (type1, type1)->0, (type1, type2)->1, (type2, type2)->2
    for (int j = i; j < ntypes; j++) {  // this rule is different form one used in the current code that would be:
                                        // (type1, type1)->0, (type2, type2)->1, (type1, type2)->2
...
Potential_EAM_tab::Potential_EAM_tab(const std::string &filename, int id1_, int id2_) {
  ...
  in >> ntypes;
  if (id2 > ntypes) {
    std::cout << "The number of materials specified in " << filename << " is less than passed index = " << id2
              << std::endl;
    return;
  }
...
Potential_SW::Potential_SW()
    : sigma(1.0), epsilon(0.0), A(0.0), B(0.0), gamma(0.0), lamda(0.0), p(0.0), q(0.0) {}  //set default values

void Potential_SW::set_parameters(Material El1,
                                  Material El2) {        // assign parameters for a given pair of elements (El1, El2)
  if ((El1 == Material::Si) && (El2 == Material::Si)) {  // F.H.Stillinger and T.A.Weber, Phys.Rev.B 31, 5262-5271
                                                         // (1985)
    name = "Si-Si";
    mass = 28.0855;
    epsilon = 2.167222;
    ...
    cut_off = 3.8437;
    cos_c = -1.0 / 3.0;
    lamda_p = sqrt(31.0 * 21.0);  // sqrt(lamda_Si*lamda_Ge)
  }
}
...
void Potential_SW::build() {
  ...
  for (int i = 0; i < n_points - 1; i++) {
    ...
    dU[i] = -A * (B * p * pow(rs, p + 1.0) - q * pow(rs, q + 1.0)) * exp(sigma / (r - cut_off)) / sigma -
            U[i] * sigma / pow(r - cut_off, 2);
  }
  U[n_points - 1] = 0.0;
  dU[n_points - 1] = 0.0;      // set the last point manually, otherwise there will be dividing by zero
  U_table.set(U, dU, r1, r2);  // build U table
  U_table.set_name("U_" + name);
  ...
  tb_f[n_points - 1] = 0.0;
  tb_df[n_points - 1] = 0.0;            // set the last point manually, otherwise there will be dividing by zero
