...
int SimulationSetup::set(Simulation & simulation) const{		//build "simulation" based on the loaded information
	...
	switch (kflag) {											//add handlers to the simulation to reproduce behaviour corresponding to a particular value of kflag
	case 1: {					//Quench
...
bool SimulationSetup::is_open() const {
	return open_flag;
}

std::string SimulationSetup::get_input() const {
	return input_snapshot;
}

std::string SimulationSetup::get_output() const {
	return output_snapshot;
}

std::string SimulationSetup::get_output_dir() const {
	return output_dir;
}

std::string SimulationSetup::get_statistics() const {
	return simulation_statistics;
}

int SimulationSetup::get_nsteps() const {
	return nstep;
}

int SimulationSetup::get_nprint() const {
	return neprt;
}

int SimulationSetup::get_nwrite() const {
	return nwrite;
}
...
void read_line(std::istream &in, T& val) {		//template is used to allow the compiler to generate a code for both int and double "val" 
	std::string buf;
...
std::string step_to_str(int val) {							//Convert "val" into a string with adding leading zeros to match the name convention from the Fortran version of MD code
	int length = 0;
	if (val < 1000) length = 3;								//select the length of the returned string
	else if (val < 10000) length = 4;
	else if (val < 100000) length = 5;
	else return std::string();

	std::stringstream ss;
	ss << std::setw(length) << std::setfill('0') << val;	//convert "val" into string of length "length" with adding leading zeroes if needed
	return ss.str();
};
