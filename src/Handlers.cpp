...
void Handler_Base::update(System& system) {
  if (active && (system.step % update_n == 0))
    update_impl(system);  // call update_impl(system) every "update_n" steps if handler is active
}

void Handler_Base::activate() { active = true; }

void Handler_Base::deactivate() { active = false; }

void Handler_Base::set_update_frequency(int n) { update_n = n; }
  ...
  if (system.step == 0) {  // assign velocities only at the 0th step
    ...
    for (int i = 0; i < ntypes; i++)
      distributions[i] = Distribution(
          0.0,
          sqrt(2.0 * kb * T0 /
               (system.masses[i] * enunit)));  // initialize velocity distribution functions
    ...
    for (int i = 0; i < n; i++) {
      auto& distribution = distributions[particles[i].type - 1];  // an abbreviated name; particle type ids start from 1
                                                                  // wile indexes in array start form 0, therefore "-1"
      for (int j = 0; j < DPVector3::dim; j++)
        particles[i].vel.data[j] =
            distribution(random_generator);  // assign the initial velocity according to the normal distributions
    }
    ...
    for (int i = 0; i < n; i++)
      particles[i].vel -=
          p_avr /
          system.masses[particles[i].type - 1];  // correct the momentum to make sure that the total momentum is zero
    ...
    for (int i = 0; i < n; i++) {
      double m =
          system.masses[particles[i].type -
                        1];  // mass; particle type ids start from 1 wile indexes in array start form 0, therefore "-1"
    ...
    for (int i = 0; i < n; i++) {
      particles[i].vel *=
          correction;  // correct the kinetic energy to make sure that energy corresponding to T0 is assigned
...
double Handler_Energy::get_Ep() const { return Ep; }

double Handler_Energy::get_Ek() const { return Ek; }

double Handler_Energy::get_Etot() const { return Etot; }

// Handler_Temperature==================================================================================================

Handler_Temperature::Handler_Temperature() : Handler_Base(), T(0.0) {}

void Handler_Temperature::update_impl(
    System& system) {  // Calculate the Temperature [K], estimated based on the average atomic kinetic energy
  double Ek = 0;
  T = 0.0;
  ...
  int n_count = 0;
  for (int i = 0; i < n; i++) {
    if (system.rigid(i) || exclude(particles[i].khist))
      continue;             // exclude "rigid" particles and particles from the exclusion list
...
double Handler_Temperature::get_T() const { return T; }

void Handler_Temperature::exclude_groups(const std::vector<int>& list) { exclude_list = list; }
...
void Handler_Pressure::update_impl(System& system) {  // calculate the Pressure based on the Virial theorem
  P = 0.0;
  Pcomponents = DPVector3();
  for (auto& particle : system.particles) {  // loop over all particles
    auto& stress = particle.stress;  // stress corresponding to each atom is already calculated in Simulation.cpp with
                                     // applying Virial theorem
    if (system.dim == 3)
      P -= (stress.x + stress.y + stress.z) / 3.0;  // the only thing that has to be done is summing everything together
    else
      P -= 0.5 * (stress.x + stress.y);
    Pcomponents -= stress;  // scale velocities
  }
}

double Handler_Pressure::get_P() const { return P; }

DPVector3 Handler_Pressure::get_Pcomponents() const { return Pcomponents; }
...
void Handler_BerendsenT::update_impl(System& system) {  // update particle velocities to maintain T0
  Handler_Temperature::update_impl(
      system);  // call update_impl(system) from Handler_Temperature to get the value of the current temperature
...
void Handler_BerendsenT::set_T(double T) { T0 = T; }
...
Handler_BerendsenP::Handler_BerendsenP(double P, Mode mode_, double beta_)
    : Handler_Pressure(), P0(P), beta(beta_), mode(mode_) {
  std::cout << info() << std::endl;
}

void Handler_BerendsenP::update_impl(System& system) {  // update system dimensions to maintain P0
  Handler_Pressure::update_impl(
      system);  // call update_impl(system) from Handler_Pressure to get the value of the current pressure
  ...
  switch (mode) {    // select the method of pressure control
    case Mode::XYZ:  // X, Y, and Z sizes are scaled together
      ...
      scale.z = d;
      break;
    case Mode::XY_Zindependent:  // X and Y sizes are controlled together based on 0.5*(Pxx + Pyy), and Z size is
                                 // controlled based on Pzz
  ...
  for (int j = 0; j < 3; j++) {
    if (system.boundaries[j] != Boundary::periodic)
      continue;  // perform pressure control only if a periodic boundary condition is applied in a particular direction
...
void Handler_BerendsenP::set_P(double P) { P0 = P; }
...
Handler_Quench::Handler_Quench() { std::cout << info() << std::endl; }

std::string MSE6270_MD::Handler_Quench::info() const { return "Quench algorithm"; }

void Handler_Quench::update_impl(System& system) {  // quench atomic motion
  auto& particles = system.particles;               // an abbreviated name
  int n = particles.size();                         // number of particles
  if (Ek_prev.size() != particles.size())
    Ek_prev.resize(n);  // On the first iteration, set Ek_prev equal to the current Ek
