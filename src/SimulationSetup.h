...
class SimulationSetup {
 public:
  SimulationSetup(
      const std::string& filelist = "md.rc");  //!< Initialize the setup based on the files listed in "filelist" file
  ...
  std::string get_output_dir()
      const;  //!< Return the name of a directory used for saving atomic snapshots (should be created manually)
  std::string get_statistics() const;  //!< Return the name of a file used for writing simulation statistics: Step,
                                       //!< Time, Energy, Kinetic, Potential, Temperature, Pressure
 ...
 private:
  void set_names(int id, const std::string& name);  // Set file names according to the convention used in the Fortran
                                                    // version of MD code, id - file id, name - file name
...
/*! Convert "val" into a string with adding leading zeros to match the name convention from the Fortran version of MD
 * code */
