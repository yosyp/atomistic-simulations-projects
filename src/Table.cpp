...
//Set arrays representing tabulated values of function "f(x)" and its derivative "df(x)/dx". Argument(x) is assumed to be uniformally distributed within arg_min and arg_max
int Table::set(const std::vector<double>& f, const std::vector<double>& df, double arg_min_, double arg_max_) {
	int n = f.size(); if ((n < 1) || (df.size() != n)) return EXIT_FAILURE;
	arg_min = arg_min_; arg_max = arg_max_;									//assign arg_min, and arg_max
	data.resize(n);
	d_data.resize(n);
...
void Table::set_name(const std::string & name_) {
	name = name_;
}
...
void Table::load(std::istream & in){										//load table from a file stream "in"
	...
	//std::cout << "loading " << name << " table: Np = " << n << ", arg_min = " << arg_min << ", arg_max = " << arg_max << std::endl;
