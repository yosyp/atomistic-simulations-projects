...
void Integrator_Base::pre_step() {
}  // perform a pre step of integration used in such algorithms like predictor-corrector
   // Declare an empty method since it can also be empty in children classes.
   // So, no multiple overriding of a purely virtual function is needed.
...
void Integrator_Verlet::step() {  // perform a step of integration
  ...
  for (int i = 0; i < n; i++) {
    ...
    double m = system->masses[particles[i].type - 1];  // particle mass; particle type ids start from 1 wile indexes in
                                                       // array start form 0, therefore "-1"
    ...
    particles[i].vel += 0.5 * dt * a;                                  // velocity at t+1/2 step
                                       // This implementation allows to avoid using additional temporal variable for
                                       // storing values on the previous step  and performing a prestep before force
                                       // calculation. However, the velocity outside the integrator is shifted by a half
                                       // step.
  }
...
void Integrator_Nord5::init() {
  ...
  q.resize(system->particles.size(),
           {DPVector3(), DPVector3(), DPVector3(),
            DPVector3()});  // resize array high order derivatives according to the number of particles
}
...
void Integrator_Nord5::pre_step() {  // perform a pre step of integration(before force calculation): predictor step
  ...
  for (int i = 0; i < n; i++) {
    ...
    particles[i].pos += particles[i].vel * dt + q[i][0] + q[i][1] + q[i][2] +
                        q[i][3];  // Update high order derivatives. Since v has units [A/dt], it multiplied by dt.
    particles[i].vel += (2.0 * q[i][0] + 3.0 * q[i][1] + 4.0 * q[i][2] + 5.0 * q[i][3]) /
                        dt;  // qi_next = sum(C_ij*qj). The sum goes from i to 4, and C_ij - is a binomial coefficient.
    q[i][0] += 3.0 * q[i][1] + 6.0 * q[i][2] +
               10.0 * q[i][3];  // Since position and velocity are set, index of q is shifted by 2.
    q[i][1] +=
        4.0 * q[i][2] + 10.0 * q[i][3];  // Check lecture notes for more details on the predictor-corrector algorithm
    q[i][2] += 5.0 * q[i][3];
  }
}
...
void Integrator_Nord5::step() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    double m = system->masses[particles[i].type - 1];  // particle mass; particle type ids start from 1 wile indexes in
                                                       // array start form 0, therefore "-1"
    DPVector3 correction = 0.5 * dt * dt / (m * enunit) * particles[i].force -
                           q[i][0];  // correction calculated based on the calculated value of force
    if (system->dim == 2) {          // in 2D simulation particles cannot move along z
      for (int j = 0; j < 4; j++) q[i][j].z = 0.0;
      particles[i].vel.z = 0.0;
      correction.z = 0.0;
...
std::string Integrator_Nord5::info() const {  // return a string with information about an integration algorithm
  return "Nord5 (5th order predictor-corrector integration algorithm): time step = " + std::to_string(system->dt) +
         " ps";
}
