...
* @date 22 Jan 2018
* @brief The file describes classes-containers used in the simulations, including DPVector3 (3D double precision vector), Particle (information about particles),
   and System (information about a particular computational system simulated with MD)*/
...
namespace MSE6270_MD {
	...
	public:
		...
		int get_interaction_id(int id1, int id2) const;					/*! Return the interaction id for a (id1, id2) pair. 
																			For example, if nptypes = 2, (1,1) -> 0, (2,2) -> 1, (1,2) -> 2.
																			If nptypes = 3, (1,1) -> 0, (2,2) -> 1, (3,3) -> 2, (1,2) -> 3, (1,3) -> 4, (2,3) -> 5. */
		int get_n_interactions() const;									//!< Return the number of interaction types for the given number of particle types: (type1, type1), (type2, type2), (type1, type2), etc.
		...
		std::vector<Potential_ptr> potentials;							/*!< An array of smart pointers to potentials used in the simulation. The potential must be assigned for each cross interection type.
																			 Different potential types can be used together. Null pointer corresponds to noninteracting particles */
	...
	inline const DPVector3 DPVector3::operator * (double rhs) const {
		return DPVector3(*this) *= rhs;
	}
	...
	inline DPVector3 MSE6270_MD::operator * (double lhs, const DPVector3 & rhs) {
		return rhs * lhs;
	}
	...
	inline double DPVector3::length() const {
		return sqrt(length_s());
	}

	inline int System::get_interaction_id(int t1, int t2) const {		//return id of a given interatomic interaction, id1 and id2 are ids of particle1 and particle2 in the considered pair of atoms
