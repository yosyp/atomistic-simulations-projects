...
/*! A class that implements a general behaviour of interatomic interaction potential. Children classes are responsible
 * for assignment of values to the tables  and setting corresponding flags. */
class Potential_Base {
 public:
  ...
  void print()
      const;  //!< Print tables into a file: r[A], U[eV], dU[eV/A]... check function implementation for more details
...
class Potential_LJ : public Potential_Base {  // expand functionality of Potential_Base
 public:
  ...
  Potential_LJ(Material material,
               double Rcut =
                   2.5);  //!< Initialize Potential_LJ for a given "material", Rcut is cutoff distance in units of sigma
  Potential_LJ(double epsilon, double sigma, const std::string& name,
               double Rcut = 2.5);    /**< Initialize Potential_LJ for given "epsilon" and "sigma",
...
class Potential_LJ_cut : public Potential_LJ {
 public:
  ...
  Potential_LJ_cut(Material material, double Rcut = 2.5);  //!< Initialize Potential_LJ_cut for a given "material", Rcut
                                                           //!< is cutoff distance in units of sigma
  Potential_LJ_cut(
      double epsilon, double sigma, const std::string& name,
      double Rcut = 2.5);                     /**< Initialize Potential_LJ for given "epsilon" and "sigma",
                                               * "name" is the name of the interaction, Rcut is cutoff distance in units of sigma
                                               * (2.5 is the default value)*/
  virtual std::string info() const override;  //!< Return a string with information about the potential
 protected:
  virtual void build()
      override;  //!< Assign values of U and dU tables for given LJ potential parameters with using cutoff function
};

/*! A class generating a pair potential based on a *.tab table. Checke Table::load for more details. Do not forget to
 * assign correct a mass to the system when set the potential */
...
class Potential_Pair_tab : public Potential_Base {  // expand functionality of Potential_Base
 public:
  Potential_Pair_tab(
      const std::string& filename);  //!< Initialize Potential_Pair based on a table from a file with a name "filename"
  virtual ~Potential_Pair_tab() = default;    // destructor must be virtual if inheritance is used
  virtual std::string info() const override;  //!< Return a string with information about the potential
};

/*! A class generating EAM potential based on *.alloy table. It is LAMMPS format, and tables for different potentials
   are available online. You may check https://www.ctcms.nist.gov/potentials/ to find the tables. But before using, try
   to plot a loaded potential to make sure that everything is Ok. Also, you may compare corresponding cohesive energies
   and lattice parameters at zero temperature with ones given on the web site. */
...
/*	The format of *.tab files :
        ...
        -----> N blocks for each type
                ...
                (atomic_volume and material_id are not used in the current code and introduced for backward
   compatibility of the table format) U : table name, number of points(np), arg_min, arg_max U_data(np) : r, U, dU / dr
...
inline double Table::min() const { return arg_min; }

inline double Table::max() const { return arg_max; }

inline const Table& Potential_Base::U() const { return U_table; }

inline const Table& Potential_Base::f() const { return f_table; }

inline const Table& Potential_Base::F() const { return F_table; }

inline const Table& Potential_Base::tb_f() const { return tb_f_table; }

inline const Table& Potential_Base::tb_g() const { return tb_g_table; }

inline bool Potential_Base::full_list() const { return full_list_flag; }

inline bool Potential_Base::eam() const { return eam_flag; }

inline bool Potential_Base::threebody() const { return tb_flag; }

inline double Potential_Base::r_cut() const { return cut_off; }
}  // namespace MSE6270_MD
